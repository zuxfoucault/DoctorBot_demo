    ...
    if not gfile.Exists(vocabulary_path):
        ...
        with gfile.GFile(data_path, mode="rb") as f:
            ...
            for line in f:
                ...
                label = line.strip()
                vocab[label] = 1
            label_list = [_UNK] + sorted(vocab)
            with gfile.GFile(vocabulary_path, mode="wb") as vocab_file:
                for k in label_list:
                    vocab_file.write(k + b"\n")
...
def prepare_multi_task_data(data_dir, in_vocab_size, out_vocab_size):
    ...
    data_to_token_ids(train_path + ".label", label_train_ids_path, label_path, normalize_digits=False)
    ...
    data_to_token_ids(dev_path + ".label", label_dev_ids_path, label_path, normalize_digits=False)
    ...
    data_to_token_ids(test_path + ".label", label_test_ids_path, label_path, normalize_digits=False)
